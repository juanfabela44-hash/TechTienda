 ...
 *
 * NOTA: Esta sección no tiene nada que ver con la lógica de la API de JavaScript.
 * Es para operaciones PHP/MySQL directas, si las necesitas.
 */

// #######################################################
// ### ⚠️ C O N F I G U R A C I Ó N   M Y S Q L ⚠️ ###
// #######################################################

// Variables de conexión para InfinityFree
...
if ($conn->connect_error) {
    // Esto es útil para debug, pero expone información sensible.
    // Deberías cambiarlo a un mensaje genérico en producción.
    // die("Conexión fallida: " . $conn->connect_error); 
    // Por ahora, solo imprime un error discreto para continuar con el HTML/JS
    echo "<script>console.error('Error de conexión a la base de datos PHP.');</script>";
} else {
    // Si la conexión es exitosa, se puede usar $conn para consultas como:
    // $sql = "SELECT * FROM productos LIMIT 10";
    // $result = $conn->query($sql);
    //
    // Luego puedes usar los resultados de $result dentro del HTML si lo necesitas.
    
    // Cierra la conexión al final si ya no la necesitas.
    // $conn->close();
}
...
    <title>TechMarket - Productos Tecnológicos</title>
...
            <p class="text-xs mt-1">Si despliegas en Railway, pon aquí tu URL (ej: https://tu-app.up.railway.app). Por defecto usa localhost.</p>
        </div>

                <div id="section-home" class="hidden-section">
            <h2 class="text-3xl font-bold mb-6 text-gray-800">Últimos Lanzamientos</h2>
...
            </form>
            <p class="mt-4 text-center">¿No tienes cuenta? <a href="#" onclick="navigate('register')" class="text-blue-500">Regístrate</a></p>
        </div>
...
                showToast('Usuario creado. Inicia sesión.', 'green');
                navigate('login');
